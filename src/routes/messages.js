...
// POST /api/messages/conversations — Start or get conversation for a ride
  ...
  try {
    ...
    if (ride_id) {
      const existing = await pool.query(
        'SELECT * FROM conversations WHERE ride_id = $1', [ride_id]
      );
    ...
    if (message) {
      ...
      await pool.query(
        ...
        [msgId, convo.id, rider_phone, message]
      );
      await pool.query(
        'UPDATE conversations SET last_message = $1, last_message_at = NOW() WHERE id = $2',
        [message.substring(0, 100), convo.id]
      );
    }
...
// GET /api/messages/conversations/driver/:driverId — List driver's conversations
router.get('/conversations/driver/:driverId', async (req, res) => {
  try {
    ...
    for (const c of result.rows) {
      ...
      convos.push({ ...c, unread_count: parseInt(unread.rows[0].count) });
    }
...
// GET /api/messages/conversations/rider/:phone — List rider's conversations
...
router.get('/conversations/:id/messages', async (req, res) => {
  try {
    ...
    const count = await pool.query(
      'SELECT COUNT(*) FROM messages WHERE conversation_id = $1', [req.params.id]
    );
    const result = await pool.query(
...
router.post('/conversations/:id/messages', async (req, res) => {
  try {
    ...
    const preview = msgType === 'image' ? '📷 Imaj' : msgType === 'voice' ? '🎤 Mesaj Vokal' : msgType === 'file' ? '📎 Fichye' : (content || '').substring(0, 100);

    await pool.query(
      `INSERT INTO messages (id, conversation_id, sender_type, sender_id, content, type, file_url, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`,
      [msgId, req.params.id, sender_type, sender_id || null, content || '', msgType, file_url || null]
    );
